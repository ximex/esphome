...
bool IOHomecontrolCover::has_address(uint32_t address) const {
  for (uint32_t addr : this->addresses_) {
    if (addr == address)
      return true;
  }
  return false;
...
void IOHomecontrolCover::dump_config() {
  ...
  for (uint32_t addr : this->addresses_) {
    ESP_LOGCONFIG(TAG, "  Source Address: 0x%06X%s", addr, addr == this->get_primary_address() ? " (primary)" : "");
  }
}

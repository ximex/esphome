...
void MiLightLight::dump_config() {
  ESP_LOGCONFIG(TAG,
                ...
                "  Group ID: %u\n"
                "  Remote Type: %s\n"
                "  Cold White: %.0f mireds\n"
                "  Warm White: %.0f mireds",
                this->device_id_, this->group_id_, LOG_STR_ARG(remote_type_to_string(this->remote_type_)),
                this->cold_white_mireds_, this->warm_white_mireds_);
}
...
bool MiLightLight::is_step_based_brightness_() const {
  return this->remote_type_ == RemoteType::REMOTE_TYPE_CCT || this->remote_type_ == RemoteType::REMOTE_TYPE_RGB ||
         this->remote_type_ == RemoteType::REMOTE_TYPE_FUT020;
}

bool MiLightLight::supports_disco_() const {
  return this->remote_type_ != RemoteType::REMOTE_TYPE_CCT && this->remote_type_ != RemoteType::REMOTE_TYPE_FUT091;
}

bool MiLightLight::supports_night_mode_() const {
  return this->remote_type_ != RemoteType::REMOTE_TYPE_RGB && this->remote_type_ != RemoteType::REMOTE_TYPE_FUT020;
}

bool MiLightLight::has_direct_mode_select_() const {
  return this->remote_type_ == RemoteType::REMOTE_TYPE_RGB_CCT || this->remote_type_ == RemoteType::REMOTE_TYPE_FUT089;
}
...
light::LightTraits MiLightLight::get_traits() {
  ...
  switch (this->remote_type_) {
    case RemoteType::REMOTE_TYPE_RGB_CCT:
    case RemoteType::REMOTE_TYPE_FUT089:
      traits.set_supported_color_modes({light::ColorMode::RGB, light::ColorMode::COLOR_TEMPERATURE});
      traits.set_min_mireds(this->cold_white_mireds_);
      traits.set_max_mireds(this->warm_white_mireds_);
      break;
    case RemoteType::REMOTE_TYPE_CCT:
    case RemoteType::REMOTE_TYPE_FUT091:
      traits.set_supported_color_modes({light::ColorMode::COLOR_TEMPERATURE});
      traits.set_min_mireds(this->cold_white_mireds_);
      traits.set_max_mireds(this->warm_white_mireds_);
      break;
    case RemoteType::REMOTE_TYPE_RGB:
    case RemoteType::REMOTE_TYPE_FUT020:
      traits.set_supported_color_modes({light::ColorMode::RGB});
      break;
    case RemoteType::REMOTE_TYPE_RGBW:
...
void MiLightLight::send_night_mode() {
  ...
  if (this->remote_type_ == RemoteType::REMOTE_TYPE_RGBW) {
...
static bool rgb_to_hs(float r, float g, float b, float &hue, float &saturation) {
...
void MiLightLight::write_state_rgb_(light::LightState *state, float brightness) {
  ...
  if (rgb_to_hs(r, g, b, hue, sat)) {
    ...
    if (this->remote_type_ == RemoteType::REMOTE_TYPE_RGB_CCT || this->remote_type_ == RemoteType::REMOTE_TYPE_FUT089) {
...
void MiLightLight::write_state_color_temp_(light::LightState *state, float brightness) {
  ...
  if (this->is_step_based_brightness_()) {
    ...
    if (this->remote_type_ == RemoteType::REMOTE_TYPE_FUT089) {
      cmd.white_mode = true;
      this->hub_->send_command(cmd);
      cmd.white_mode = false;
    }
...
void MiLightLight::apply_received_command(const ReceivedCommand &rx) {
  ...
  if (rx.night_mode) {
    ...
    call.set_state(true);
    call.set_effect(EFFECT_NIGHT_MODE);
    this->suppress_next_write_ = true;
    call.perform();
    return;
  }
  ...
  if (rx.has_effect) {
    ...
    if (this->has_direct_mode_select_() && rx.effect < 9) {
      call.set_effect(EFFECT_DISCO_NAMES[rx.effect]);
    } else {
      call.set_effect(EFFECT_DISCO_CYCLE);
    }
    this->suppress_next_write_ = true;
    call.perform();
    return;

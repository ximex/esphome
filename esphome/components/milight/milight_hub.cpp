...
void MiLightHub::dump_config() {
  ...
  const LogString *power_str = LOG_STR("MAX (0 dBm)");  // also covers RF24_POWER_LEVEL_MAX below
  switch (this->rf24_power_level_) {
    case Rf24PowerLevel::RF24_POWER_LEVEL_MIN:
      power_str = LOG_STR("MIN (-18 dBm)");
      break;
    case Rf24PowerLevel::RF24_POWER_LEVEL_LOW:
      power_str = LOG_STR("LOW (-12 dBm)");
      break;
    case Rf24PowerLevel::RF24_POWER_LEVEL_HIGH:
      power_str = LOG_STR("HIGH (-6 dBm)");
      break;
    case Rf24PowerLevel::RF24_POWER_LEVEL_MAX:
      break;
  }
  ESP_LOGCONFIG(TAG,
                "  Packet Repeats: %u\n"
                "  RF24 Power Level: %s\n"
                "  Packets Per Loop: %u\n"
                "  Throttle: threshold=%" PRIu32 "ms sensitivity=%u minimum=%u\n"
                "  Listen: %s\n"
                "  RX Slots: %u\n"
                "  Registered Lights: %u",
                this->packet_repeats_, LOG_STR_ARG(power_str), this->packet_repeats_per_loop_,
                this->throttle_threshold_ms_, this->throttle_sensitivity_, this->throttle_minimum_,
                YESNO(this->listen_enabled_), this->rx_slot_count_, this->registered_light_count_);
}
...
uint16_t MiLightHub::throttled_repeats_() {
  ...
  if (effective != this->packet_repeats_) {
    ESP_LOGD(TAG, "Throttled repeats: %u -> %u (elapsed=%" PRIu32 "ms)", this->packet_repeats_, effective, elapsed);
  }
  return effective;
}
...
void MiLightHub::send_unpair(uint16_t device_id, uint8_t group_id, RemoteType type) {
  ...
  if (type == RemoteType::REMOTE_TYPE_RGBW) {
    LightCommand on_cmd{};
...
void MiLightHub::dispatch_rx_packet_(const uint8_t *payload, uint8_t length, uint8_t radio_config_index) {
  ...
  const LogString *action = LOG_STR("unknown");
  if (rx.is_on) {
    action = LOG_STR("ON");
  } else if (rx.is_off) {
    action = LOG_STR("OFF");
  } else if (rx.night_mode) {
    action = LOG_STR("night_mode");
  } else if (rx.white_mode) {
    action = LOG_STR("white_mode");
  } else if (rx.has_effect) {
    action = LOG_STR("effect");
  } else if (rx.effect_speed_up) {
    action = LOG_STR("speed_up");
  } else if (rx.effect_speed_down) {
    action = LOG_STR("speed_down");
  } else if (rx.has_brightness) {
    action = LOG_STR("brightness");
  } else if (rx.has_color_temp) {
    action = LOG_STR("color_temp");
  } else if (rx.has_hue) {
    action = LOG_STR("color");
  } else if (rx.step_up_brightness) {
    action = LOG_STR("step_bright_up");
  } else if (rx.step_down_brightness) {
    action = LOG_STR("step_bright_down");
  } else if (rx.step_up_temp) {
    action = LOG_STR("step_temp_up");
  } else if (rx.step_down_temp) {
    action = LOG_STR("step_temp_down");
  }

  ESP_LOGI(TAG, "RX: type=%s dev=0x%04X group=%u seq=%u cmd=%s", LOG_STR_ARG(remote_type_to_string(rx.remote_type)),
           rx.device_id, rx.group_id, rx.sequence, LOG_STR_ARG(action));
...
void MiLightHub::init_sniff_slots_() {
  ...
  static constexpr RemoteType SNIFF_TYPES[] = {
      RemoteType::REMOTE_TYPE_RGBW, RemoteType::REMOTE_TYPE_CCT,    RemoteType::REMOTE_TYPE_RGB_CCT,
      RemoteType::REMOTE_TYPE_RGB,  RemoteType::REMOTE_TYPE_FUT020,
  };
